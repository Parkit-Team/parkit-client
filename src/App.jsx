...
function App() {
  ...
  const handleStart = async () => {
    try {
      ...
      //로컬
      const res = await fetch('http://192.168.201.98/api/driving-sessions/start', {
        method: 'POST',
  ...
  const handleStop = async () => {
    if (isRunning) {
      ...
      // 로컬
      await fetch(`http://192.168.201.98/api/driving-sessions/${sessionId}/stop`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      //로컬
      webSocketFactory: () => {
        const sock = new SockJS('http://192.168.201.98/ws/parkit');
